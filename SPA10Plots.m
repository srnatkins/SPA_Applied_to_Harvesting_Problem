...
% improve the switches to h_{10,sing} that serves as an alternative
...
% h_{10,sing}, and the values for y2(0) and lambda1(L). All of which were 
% numerically obtained in another script file- SPA10.m 
% 
% The file outputs four figures

% Figure 1 displays the resulting switching function on [0,L]

% Figure 2 displays the resulting switching function on [s3,s8]

% Figure 3 displays the resulting switching function on [s4,s7]

% Figure 4 displays h_{10,sing}^* and the state variable y1(x) = u(x)
...
h5 = 0.5;                   % h(x) = 1/2  on (s5,s6)  <- singular arc
h6 = hmax;                  % h(x) = hmax on (s6,s7)
h7 = 0;                     % h(x) = 0    on (s7,s8)
h8 = hmax;                  % h(x) = hmax on (s8,s9)
h9 = 0;                     % h(x) = 0    on (s9,s10)
h10 = hmax;                 % h(x) = hmax on (s10,s11) where s11=L
h = [h0;h1;h2;h3;h4;h5;h6;h7;h8;h9;h10];
N=length(h);
y20 =0.192080970160368;     % y2(0)
y20save = y20;
lam1L = 1.301534450976960;  % lambda1(L)
ds = L/1000;
dxstep = 1.e-3;
errtol = 1.e-10;
dxlam = 1.e-4;
% The switches obtained from using SPA
s1 = 1.768148077803724;     % sopt(1)
s2 = 3.108969753655565;     % sopt(2)
s3 = 3.432063490336991;     % sopt(3)
s4 = 3.509547482108235;     % sopt(4)
s5 = 3.524306699035758;     % sopt(5)
s6 = 6.475693466679342;     % sopt(6)
s7 = 6.490452616353865;     % sopt(7)
s8 = 6.567936525559686;     % sopt(8)
s9 = 6.891030246835277;     % sopt(9)
s10 =8.231851922255341;     % sopt(10)
...
for k =1:N
    ...
    Ynew = Y{k};
    y1{k} = Ynew(:,1);
    htest{k} = h(k)*ones(length(Ynew(:,1)),1);
...
hold on 
plot(xstep{8},psi{8},'k', 'Linewidth',2);
hold on 
plot(xstep{9}, psi{9},'k', 'Linewidth',2);
hold on 
plot(xstep{10}, psi{10},'k', 'Linewidth',2);
hold on
plot(xstep{11}, psi{11},'k', 'Linewidth',2);
hold on
xline(s(1),'--r');
hold on 
xline(s(2),'--r');
...
title('Switching Function ($h_{10,sing}^*$)', 'Interpreter', 'latex', 'FontSize', 25);
ax = gca;
ax.FontSize = 20;
figure(2)
plot(xstep{4},psi{4},'k', 'Linewidth',2);
...
xline(s(8),'--r');
xlim([s(3) s(8)])
xlabel('Habitat Subinterval  $[s_3^*,s_8^*]$','Interpreter','latex','FontSize', 25)
ylabel('$\psi(x)$','Interpreter', 'latex', 'FontSize',25);
title('Switching Function ($h_{10,sing}^*$)', 'Interpreter', 'latex', 'FontSize', 25);
ax = gca;
ax.FontSize = 20;
%savefig('SPA6PsiPlot.fig');
...
xline(s(7),'--r');
xlim([s(4) s(7)])
xlabel('Habitat Subinterval $[s^*_4,s^*_7]$','Interpreter','latex','FontSize', 25)
ylabel('$\psi(x)$','Interpreter','latex','FontSize',25);
title('Switching Function ($h_{10,sing}^*$)','Interpreter','latex','FontSize', 20);
ax = gca;
ax.FontSize = 20;
figure(4)
plot(xtest{1},y1{1},'b','Linewidth',2)
hold on
plot(xtest{1},htest{1},'r','Linewidth',1.5)
hold on
plot(xtest{2},y1{2}, 'b','Linewidth',2)
hold on
plot(xtest{2},htest{2}, 'r','Linewidth',1.5)
hold on
plot(xtest{3},y1{3},'b','Linewidth',2)
hold on 
plot(xtest{3},htest{3}','r','Linewidth',1.5)
hold on 
plot(xtest{4},y1{4},'b','Linewidth',2)
hold on 
plot(xtest{4},htest{4},'r','Linewidth',1.5)
hold on 
plot(xtest{5}, y1{5},'b','Linewidth',2)
hold on 
plot(xtest{5}, htest{5},'r','Linewidth',1.5)
hold on
plot(xtest{6},y1{6},'b','Linewidth',2)
hold on 
plot(xtest{6}, htest{6},'r','Linewidth',1.5)
hold on 
plot(xtest{7},y1{7},'b','Linewidth',2)
hold on 
plot(xtest{7}, htest{7},'r','Linewidth',1.5)
hold on 
plot(xtest{8},y1{8},'b','Linewidth',2)
hold on 
plot(xtest{8}, htest{8},'r','Linewidth',1.5)
hold on 
plot(xtest{9},y1{9},'b','Linewidth',2)
hold on 
plot(xtest{9}, htest{9},'r','Linewidth',2)
hold on 
plot(xtest{10}, y1{10}, 'b', 'Linewidth', 2)
hold on 
plot(xtest{10}, htest{10},'r','Linewidth',2)
hold on 
plot(xtest{11}, y1{11}, 'b', 'Linewidth', 2)
hold on 
plot(xtest{11}, htest{11},'r','Linewidth',2)
hold on
% ploting vertical line segments - jump discontinuities in h
...
hold on 
plot([s(5),s(5)],[0,1],'r','Linewidth', 2);
hold on 
...
hold on 
plot([s(9),s(9)], [0,1], 'r', 'Linewidth', 2);
hold on 
plot([s(10),s(10)],[0,1], 'r', 'Linewidth', 2);
ylim([-.0001,1.001])
xlim([0,10])
xlabel('Habitat Location $x$','Interpreter','latex','FontSize', 25)
title('$h_{10,sing}^*$ and $y_1^*$', 'Interpreter', 'latex', 'FontSize', 25);
ax = gca;
ax.FontSize = 20;
