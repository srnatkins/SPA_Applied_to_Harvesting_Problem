...
% improve the switches to h_{6,bang} that serves as an alternative
...
% h_{6,bang}, and the values for y2(0) and lambda1(L). All of which were 
% numerically obtained in another script file- SPA6Bang.m
% 
% The file outputs 3 figures

% Figure 1 displays the switching function

% Figure 2 displays the switching funciton over interval [s2,s5]

% Figure 3 displays h_{6,bang}^* and the resulting state solution
% y1(x)=u(x)
...
h5 = 0;                    % h(x) = 0    on (s5,s6)
h6 = hmax;                 % h(x) = hmax on (s6,s7) where s7=L

h = [h0; h1; h2; h3; h4; h5; h6];          
N = length(h);      
y20 = 0.190046980525419;   % y2(0) 
lam1L = 1.301645948218284; % lambda1(L)
ds=L/1200;                 % not needed for the code
dxstep = 1.e-3;
errtol =1.e-10;
dxlam = dxstep;
% The switches obtained from using SPA
s1 = 1.825126242423269;         % sopt(1)                     
s2 = 3.415468322287929;         % sopt(2)
s3 = 4.500945500521667;         % sopt(3)
s4 = 5.499054503334808;         % sopt(4)
s5 = 6.584531679498800;         % sopt(5)
s6 = 8.174873758208648;         % sopt(6)
s = [s1;s2;s3;s4;s5;s6];
xN = 200;
...
title('Switching Function ($h_{6,bang}^*$)', 'Interpreter', 'latex', 'FontSize', 25);
ax = gca;
ax.FontSize = 20;
%savefig('SPA6BangPsiPlot.fig');
...
hold on 
xline(s(5),'--r');
xlabel('Habitat Subinterval $[s^*_2,s^*_5]$','Interpreter','latex','FontSize', 25)
ylabel('$\psi(x)$','Interpreter','latex','FontSize',25);
title('Switching Function ($h_{6,bang}^*$)','Interpreter','latex','FontSize', 20);
ax = gca;
ax.FontSize = 20;
%savefig('SPABangZoom.fig');
...
hold on 
% plotting vertical line segments- the jump discontinuities of the control
plot([s(1),s(1)],[0,1],'r', 'Linewidth', 1.5);
hold on 
plot([s(2),s(2)],[0,1],'r', 'Linewidth', 1.5);
hold on 
plot([s(3),s(3)],[0,1],'r', 'Linewidth', 1.5);
hold on 
plot([s(4),s(4)],[0,1],'r', 'Linewidth', 1.5);
hold on 
plot([s(5),s(5)],[0,1],'r','Linewidth', 1.5);
hold on 
plot([s(6),s(6)],[0,1],'r','Linewidth',1.5);
ylim([-.001,1.001])
xlabel('Habitat Location $x$','Interpreter','latex','FontSize', 25)
title('$h_{6,bang}^*$ and $y_1^*$', 'Interpreter', 'latex', 'FontSize', 25);
ax = gca;
ax.FontSize = 20;
%savefig('SPA6BangPlot.fig');
