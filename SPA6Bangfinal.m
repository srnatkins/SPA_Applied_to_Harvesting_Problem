...
% a switch point algorithm in the case where the habitat length is 10 
% and the parameter hmax=1. In this file we assume the control form has 
% 6 switches with no singular arc.

% The initial guess of the switches, y2(0), and lambda1(L) came from
% SPA6BangInitialGuess.m which takes the switches obtained from SPA6.m
% and adjust the values to where the corresponding state y1(x) is
% physically relevant. 
...
N = length(h);      
y20 = 0.226106983650167;
y20save = y20;
lam1L =  1.112626597870163;
ds=L/1200;                   
dxstep = 1.e-3;
errtol =1.e-10;
dxlam = dxstep;
...
s1 = 1.808121762005436;                             
s2 = 3.063103235269388;                 
s3 = 3.397266684882776;                
s4 = 6.602733319818394;                
s5 = 6.936896766723736;
s6 = 8.191878237927705;

s = [s1;s2;s3;s4;s5;s6];
ssave = [s1;s2;s3;s4;s5;s6];
olds = s;
...
%interior of [s^{k}_i-ds,s^{k}_i+ds] for all i=1,...,6 or until a maximum 
