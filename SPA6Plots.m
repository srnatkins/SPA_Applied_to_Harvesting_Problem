...
% improve the switches to h_{6,sing} that serves as an alternative
...
% h_{6,sing}, and the values for y2(0) and lambda1(L). All of which were 
% numerically obtained in another script file- SPA6.m 
% 
% The file outputs 3 figures

% Figure 1 displays the resulting switching function and vertical lines
% indicating the locations of the switches. 

% Figure 2 is the switching function zoomed in near the singular region

% Figure 3 displays h_{4,sing}^* and the resulting state solution
% y1(x)=u(x)
...
h3= 0.5;                   % h(x) = 1/2  on (s3,s4) <- singular arc
...
N = length(h);      
y20 = 0.192080841873656;     % y2(0)
lam1L = 1.301535317935051;   % lambda1(L)
ds=L/100;
dxstep = 1.e-3;
errtol =1.e-10;
dxlam = dxstep;
% The switches obtained from using SPA
s1 = 1.768121762005436;                 % sopt(1)              
s2 = 3.103103235269388;                 % sopt(2)
s3 = 3.357266684882776;                 % sopt(3) 
s4 = 6.642733319818394;                 % sopt(4) 
s5 = 6.896896766723736;                 % sopt(5)
s6 = 8.231878237927704;                 % sopt(6)
s = [s1;s2;s3;s4;s5;s6];
xN = 200;
...
title('Switching Function ($h_{6,sing}^*$)', 'Interpreter', 'latex', 'FontSize', 25);
ax = gca;
ax.FontSize = 20;
...
ylabel('$\psi(x)$','Interpreter','latex','FontSize',25);
title('Switching Function ($h_{6,sing}^*$)','Interpreter','latex','FontSize', 20);
ax = gca;
ax.FontSize = 20;
%savefig('SPA6psizoom.fig');
...
hold on 
plot([s(3),s(3)],[0.5,1],'r', 'Linewidth', 1.5);
hold on 
plot([s(4),s(4)],[0.5,1],'r', 'Linewidth', 1.5);
hold on 
...
ylim([-.001,1.001])
xlabel('Habitat Location $x$','Interpreter','latex','FontSize', 25)
title('$h_{6,sing}^*$ and $y_1^*$', 'Interpreter', 'latex', 'FontSize', 25);
ax = gca;
ax.FontSize = 20;
%savefig('SPA6Plot.fig');
