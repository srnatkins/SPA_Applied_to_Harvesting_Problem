...
%The regularization tuning parameter is p=10^{-6}
...
uL = 0;
p =1.e-6;           % total variation penalty parameter 
...
%we want bl<= AH <= bu 
%A is used for the total variation portion of objective funtion
...
%checking if singular region is acutally 1/2
midpoint = N/2;
hmid = h(midpoint);
disp('approximate value of the control on the singular region')
disp(hmid);
znew = updatestate(Hp);
disp('state v=du/dx at 0')
disp(znew(1));
...
disp('Finding the switches.')
k=1;
%1st switch: h switches from hmax=1 to 0
while (Hp(k)>tol) % while h(x) is greater than 0 
    k = k+1;
end
x = linspace(0,L,N+1);                    % partitioning line segment [0,L]
pswitch1= x(k);                           % computing 1st switch
s1=pswitch1;                              % using variable for plotting
fprintf('first switch: %e\n', pswitch1);  % printing 1st switch
%2nd switch: h switches from 0 to 1
while(Hp(k)<hmax)                     
    k = k+1;
end
pswitch2 = x(k);                          % computing 2nd switch
s2= pswitch2;                             % using variable for plotting
fprintf('second switch: %e\n', pswitch2); % printing 2nd switch

%3rd switch: h switches from 1 to 1/2
while (Hp(k) >0.5)                       
    k = k+1;
end
pswitch3 = x(k);                          % computing 3rd switch
s3= pswitch3;                             % using variable for plotting
fprintf('third switch: %e\n', pswitch3);  % printing 3rd switch

%Intermediate switch 1
%based on the plot h switches from 1 to a value V less than 1/2
% so we will as so check when h switches from V to 1/2
while (Hp(k) < 0.5)                      % while h(x) not near 1/2
    k = k+1;
end 
midswitch1 = x(k);                       % computing midswitch1
x1= midswitch1;                          % using variable for plotting

%intermediate switch 2 
%based on plot, h switches from 1/2 to a value V less than 1/2
% so we will check when h switches from 1/2 to V
while (Hp(k) >=0.5)                     % while h is near 1/2
    k = k+1;                        
end
 midswitch2 = x(k);                      % computing midswitch2
 x2 = midswitch2;                        % using variable for plotting
% fprintf('midswitch2: %e\n', midswitch2);  

%4th switch: h switches from V to hmax=1
while (Hp(k)<hmax-tol)                   % while h is not near hmax=1
    k=k+1;
end
%fprintf('k: %e',k);                     
pswitch4 = x(k);                         % computing 4th switch
s4= pswitch4;                            % using variable for plotting
fprintf('fourth switch: %e\n',pswitch4); % print 4th switch

%5th switch: h switches from hmax to 0
while (Hp(k)>tol)
    k = k+1;
end
pswitch5 = x(k);                          % computing 5th switch
s5=pswitch5;                              % using variable for plotting
fprintf('fifth switch: %e\n',pswitch5);   % print 5th switch

%6th switch : h switches from 0 to hmax
while (Hp(k)<hmax)
    k = k+1;
end
pswitch6 = x(k);                           % computing 6th switch
s6=pswitch6;                               % using variable for plotting
fprintf('sixth switch: %e\n', pswitch6);   % printing 6th switch
...
hold on
line([s5,s5],[-0.05,0.01],'Color', 'red', 'LineStyle', '--')
hold on
line([s6,s6],[-0.05,0.01],'Color', 'red', 'LineStyle', '--')
hold on
line([x1,x1],[-0.05,0.01],'Color', 'magenta', 'LineStyle', '--')
hold on
line([x2,x2],[-0.05,0.01],'Color', 'magenta', 'LineStyle', '--')
hold on
